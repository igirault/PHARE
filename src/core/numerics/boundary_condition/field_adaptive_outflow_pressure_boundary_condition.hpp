...
public:
               ...
               typename Super::boundary_condition_context_type const& ctx) override
    {
        ...
        QtyCentering const centering
            = GridLayoutT::centering(PField.physicalQuantity())[dir_n];
        ...
        auto B1Field = ctx.accessor_new.getVecField(vector_quantity_type::B1);
        ...
        auto projectB = [&](auto const& idx) {
            double const bx = GridLayoutT::template project<GridLayoutT::faceXToCellCenter>(B1x, idx);
            double const by = GridLayoutT::template project<GridLayoutT::faceYToCellCenter>(B1y, idx);
            double const bz = GridLayoutT::template project<GridLayoutT::faceZToCellCenter>(B1z, idx);
        ...
        {
            bool const superfast = regimeForSlice_(superfastCache, index, dir_n, sign_n,
                                                   interior_n, projectB, ctx);
