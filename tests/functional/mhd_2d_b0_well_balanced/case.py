...
Any growth of |V| or |B1| is therefore 100% attributable to the way B0 is handled
in the finite-volume flux (B0 reconstructed to L/R face states -> spurious jump,
plus non-zero discrete divergence of the B0 Maxwell stress).

Two B0 modes, selected by env var PHARE_B0_MODE:
  - "dipole"  (default): smooth 2D dipole with its singularity OUTSIDE the domain,
                         so B0 is smooth but has a real gradient -> expect spurious |V|.
  - "uniform": constant B0 vector (grad B0 = 0) -> control, expect |V| ~ machine zero.

Resolution and reconstruction are overridable via env vars for the scan:
  PHARE_NCELLS   (default 128)         square grid
  PHARE_RECON    (default "WENOZ")     "WENOZ" or "Linear"
  PHARE_LIMITER  (default "None")      "None" or "VanLeer"
"""
...
# --- background field --------------------------------------------------------
# Normalized units. Dipole singularity placed OUTSIDE the [0, L]^2 domain (to the
# lower-left), so B0 is smooth throughout the domain yet keeps a sizeable gradient.
L = 4.0                      # domain side length
dipole_center = (-1.5, -1.5) # outside the domain
m_dipole = 1.0               # dipole moment (sets B0 amplitude)
B0_UNIFORM = (0.0, 0.3, 0.0) # control: constant background field
B1_UNIFORM = (0.0, 0.2, 0.0) # uniform perturbation field (IMF-in-B1 equilibrium)
...
# CFL based on a conservative fast-speed estimate (cs + cA) with max |B0| ~ a few.
B0_scale = 2.0
...
def config():
    ...
    if B1_MODE == "uniform":
        ...
        # so the magnetized fluid must stay at rest -> tests cross-term well-balancing.
        model_kwargs.update(
