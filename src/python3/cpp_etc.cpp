...
PYBIND11_MODULE(cpp_etc, m)
{
    ...
    // write_niter_period/niter_period cadence can resume on restart instead of resetting to 0.
    // Older restart files predating this attribute default to 0 (same as a fresh run).
    m.def("restart_coarse_step", [&](std::string const& path) -> std::size_t {
        _PHARE_WITH_HIGHFIVE({
            auto const& restart_file = samrai_restart_file(path);
            PHARE::hdf5::h5::HighFiveFile h5File{restart_file, HighFive::File::ReadOnly,
                                                 /*para=*/false};
            if (!h5File.file().getGroup("/phare").hasAttribute("coarse_step_index"))
                return 0;
            return h5File.read_attribute<std::size_t>("/phare", "coarse_step_index");
        });
