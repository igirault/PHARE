...
def populateDict(sim):
    ...
    # vector-potential init (2D): B0 = curl(a0z z_hat), B1 = curl(a1z z_hat). The mode
    # strings tell the C++ side which fields to build from their potential instead of from
    # the component functions above. The potential functions are always provided (zero by
    # default) so the dict keys exist regardless of mode.
