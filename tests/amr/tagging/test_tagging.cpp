...
//-----------------------------------------------------------------------------
//  Boundary mask: tags are cleared within a halo of the inner boundary
//-----------------------------------------------------------------------------
...
TEST(InnerBoundaryTagger, doesNotTagCellsNearBoundary)
{
    ...
    // Fill By with a linear ramp so the gradient pass tags every cell to 1.
    // The centered default indicator on a unit-slope ramp is
    //   |By(i+1)-By(i-1)| / (1 + 1/2(|By(i+1)-By(i)| + |By(i)-By(i-1)|)) = 2/(1+1) = 1.0 > thr
    ...
    auto dict = taggingDict("default", {{"B", 0.1}}); // gradient pass tags all cells = 1
    dict["inner_boundary_no_refinement_halo"] = halo;

    ConcreteTaggerKernel<IBTaggerMockModel> tagger{dict};
