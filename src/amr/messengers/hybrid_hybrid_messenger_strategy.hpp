...
public:
                            ...
                            std::unique_ptr<IMessengerInfo> fromFinerInfo) override
    {
        ...
        BregridAlgo.registerRefine(b_id, b_id, b_id, BRefineOp_, overwriteInteriorTFfillPattern);
     ...
     */
    void fillIonPopMomentGhosts(IonsT& ions, level_t& level, double const afterPushTime) override
    {
                   ...
                   double const prevCoarserTime, double const newCoarserTime) override
    {
        ...
        if (newCoarserTime < prevCoarserTime)
            throw std::runtime_error("Error : prevCoarserTime (" + std::to_string(prevCoarserTime)
                                     + ") should be < newCoarserTime ("
                                     + std::to_string(prevCoarserTime) + ")");
    ...
    void prepareStep(IPhysicalModel& model, level_t& level, double currentTime) override
    {
        ...
        for (auto& patch : level)
        {
            auto dataOnPatch = resourcesManager_->setOnPatch(
                *patch, hybridModel.state.electromag, hybridModel.state.J, hybridModel.state.ions,
                Jold_, NiOld_, ViOld_);
    ...
    void fillRootGhosts(IPhysicalModel& model, level_t& level, double const initDataTime) override
    {
        auto levelNumber = level.getLevelNumber();
...
private:
    void registerGhostComms_(std::unique_ptr<HybridMessengerInfo> const& info)
    {
        ...
        velPatchGhostsRefiners_.addTimeRefiners(info->ghostBulkVelocity, info->modelIonBulkVelocity,
    ...
    void registerInitComms_(std::unique_ptr<HybridMessengerInfo> const& info)
    {
        ...
        lvlGhostPartOldRefiners_.addStaticRefiners(
            info->levelGhostParticlesOld, levelGhostParticlesOldOp_, info->levelGhostParticlesOld);


        lvlGhostPartNewRefiners_.addStaticRefiners(
            info->levelGhostParticlesNew, levelGhostParticlesNewOp_, info->levelGhostParticlesNew);
    ...
    CoarsenOperator_ptr vecFieldMomentsCoarseningOp_{std::make_shared<MomentsVecFieldCoarsenOp>()};
