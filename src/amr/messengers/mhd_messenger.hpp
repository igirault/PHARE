...
public:
                            ...
                            [[maybe_unused]] std::unique_ptr<IMessengerInfo> fromFinerInfo) override
    {
        ...
        if (!b_id)
        {
            throw std::runtime_error("MHDMessengerStrategy: missing magnetic field variable IDs");
        }
        ...
        if (!e_id)
        {
            throw std::runtime_error("MHDMessengerStrategy: missing electric field variable IDs");
        }
        ...
        if constexpr (dimension >= 2)
        {
            ...
            if constexpr (dimension == 3)
            {
                ...
                HydroZpatchGhostRefluxedAlgo.registerRefine(*rho_fz_reflux_id, *rho_fz_reflux_id,
                                                            *rho_fz_reflux_id, mhdFluxRefineOp_,
                                                            nonOverwriteInteriorTFfillPattern);
                HydroZpatchGhostRefluxedAlgo.registerRefine(*rhoV_fz_reflux_id, *rhoV_fz_reflux_id,
                                                            *rhoV_fz_reflux_id, mhdVecFluxRefineOp_,
                                                            nonOverwriteInteriorTFfillPattern);
                HydroZpatchGhostRefluxedAlgo.registerRefine(*Etot_fz_reflux_id, *Etot_fz_reflux_id,
        ...
        {
            throw std::runtime_error("MHDMessenger: missing electric refluxing field variable IDs");
        }
        ...
        EpatchGhostRefluxedAlgo.registerRefine(*e_reflux_id, *e_reflux_id, *e_reflux_id,
                                               EfieldRefineOp_, nonOverwriteInteriorTFfillPattern);
                       ...
                       int const levelNumber) override
    {
        ...
        EpatchGhostRefluxedSchedules[levelNumber] = EpatchGhostRefluxedAlgo.createSchedule(level);
                     ...
                     double currentTime) final
    {
        ...
        for (auto& patch : level)
        {
            auto dataOnPatch = resourcesManager_->setOnPatch(
                *patch, mhdModel.state.rho, mhdModel.state.V, mhdModel.state.P, mhdModel.state.rhoV,
                mhdModel.state.Etot, mhdModel.state.J, rhoOld_, Vold_, Pold_, rhoVold_, EtotOld_,
                Jold_);
...
private:
    ...
    using FieldRefinePatchStrategyList = std::vector<std::shared_ptr<FieldRefinePatchStrategyT>>;
    ...
    std::map<int, std::shared_ptr<SAMRAI::xfer::RefineSchedule>> HydroXpatchGhostRefluxedSchedules;
    std::map<int, std::shared_ptr<SAMRAI::xfer::RefineSchedule>> HydroYpatchGhostRefluxedSchedules;
    std::map<int, std::shared_ptr<SAMRAI::xfer::RefineSchedule>> HydroZpatchGhostRefluxedSchedules;
    ...
    using VecFieldTimeInterp = VecFieldLinearTimeInterpolate<GridLayoutT, GridT, core::MHDQuantity>;
