...
struct Collector : ::benchmark::ConsoleReporter
{
    ...
    Collector(bool const quiet, bool const markdown)
        : quiet_{quiet}
        , markdown_{markdown}
    {
    }
    ...
    // the same tables as github-flavoured markdown, for pasting into a PR or a README
    void print_markdown(std::ostream& os) const
    {
        ...
        for (auto const& group : groups_)
        {
            ...
            os << "\n| **nodes** |";
            for (auto const c : cells)
                os << " " << human(nodes_.at({group, "", c})) << " |";

            for (auto const& variant : variant_order)
            {
                if (!std::any_of(cells.begin(), cells.end(),
                                 [&](auto const c) { return times_.count({group, variant, c}); }))
                    continue;
                ...
                for (auto const c : cells)
                {
                    ...
                    auto const ref = times_.find({group, reference_variant, c});
                    os << " " << time_str(it->second);
                    if (ref != times_.end() && variant != reference_variant)
                        os << " (" << delta_str(it->second, ref->second) << ")";
                    os << " |";
                }
            }
            os << "\n";
...
int main(int argc, char** argv)
{
    ...
    // ours, not googlebench's: drop them before Initialize sees them
    bool quiet = false, markdown = false;
    for (int i = 1; i < argc; ++i)
    {
        std::string const arg{argv[i]};
        if (arg != "--summary-only" && arg != "--markdown")
            continue;

        quiet |= arg == "--summary-only";
    ...
    PHARE::bench::Collector collector{quiet, markdown};
