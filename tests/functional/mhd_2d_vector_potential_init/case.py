#!/usr/bin/env python3
"""
Vector-potential init test for the MHD B = B0 + B1 splitting (2D).
...
divergence div B = dx Bx + dy By is generically nonzero. When prescribed through an
out-of-plane vector potential A_z (a0z / a1z), B = curl(A_z z_hat) is built with the same
discrete curl as Faraday, so the discrete div B = 0 to machine precision.

The exact solution is REST (V = 0). Selected by env var PHARE_B_INIT:
  - "potential" (default): B0 from a0z, B1 from a1z  -> expect div B ~ machine zero
  - "components":          B0 from b0x/b0y, B1 from b1x/b1y -> expect div B != 0
...
def psi0(x, y):
    # B0 = curl(psi0 z_hat): Bx = d psi0/dy = b0x, By = -d psi0/dx = b0y
    xr, yr = _xr(x, y), _yr(x, y)
...
def config():
    ...
    if B_INIT == "potential":
        ...
        ph.MHDModel(
            b0x=dipole_b0x, b0y=dipole_b0y, b1x=imf_b1x, b1y=imf_b1y, **common
        )
    else:
