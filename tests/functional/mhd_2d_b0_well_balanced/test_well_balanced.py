...
Cases:
  ...
  - dipole B0, B1 = uniform  : cross-term (j0xB1) well-balancing, IMF-in-B1 convention
...
def _max_abs_v(diag_dir):
    r = Run(os.path.join(HERE, diag_dir))
    m = 0.0
    for t in r.times("mhd_V"):
        h = r._get_hier_for(float(t), "mhd_V")
