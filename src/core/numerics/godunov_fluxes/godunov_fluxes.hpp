...
public:
     ...
     */
    template<typename CT, typename MeshData, typename State, typename Fluxes>
    void degrade_fluxes_near_inner_boundary(CT& ct, MeshData& meshData, State& state,
                                            Fluxes& fluxes)
    {
        if (!this->hasLayout())
            throw std::runtime_error(
                "Error - GodunovFluxes::degrade_fluxes_near_inner_boundary - GridLayout not set");
        ...
        for_N<num_directions>([&](auto i) {
            ...
            for (auto const& elem : degraded)
            {
                ...
                ct.template save<direction>(riemann_.vt, riemann_.uct_coefs, idx);
            }
        });
    }
